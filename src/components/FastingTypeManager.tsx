...
const FastingTypeManager = () => {
    ...
    const handleSave = () => {
        ...
        if (slots.length === 0) return alert(t('addAtLeastOneSlot'));
    ...
    const formatTime = (timeStr: string) => {
        ...
        if (parts.length === 3) {
            ...
            return `${t('day')} ${day} ${parts[1]}:${parts[2]}`;
        }
        return timeStr;
    };
        ...
        <div style={{ marginTop: 'var(--space-lg)' }}>
            ...
            {isCreating && (
                <div style={{ background: 'var(--c-bg-app)', padding: 'var(--space-md)', borderRadius: 'var(--radius-md)', marginBottom: 'var(--space-md)', border: '1px solid #ddd' }}>
                    <h4 style={{ marginTop: 0 }}>{editingTypeId ? t('editFastingType') : t('createFastingType')}</h4>

                    <div style={{ marginBottom: 'var(--space-md)' }}>
                        <label style={{ display: 'block', fontSize: '0.9rem', marginBottom: '4px' }}>{t('name')}</label>
                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} style={{ width: '100%', padding: '8px', border: '1px solid #ccc', borderRadius: 'var(--radius-md)' }} placeholder={t('namePlaceholder')} />
                    </div>

                    <div style={{ marginBottom: 'var(--space-md)' }}>
                        <label style={{ display: 'block', fontSize: '0.9rem', marginBottom: '4px' }}>{t('colorForCalendar')}</label>
                    ...
                    <div style={{ marginBottom: 'var(--space-md)' }}>
                        <label style={{ display: 'block', fontSize: '0.9rem', marginBottom: '4px' }}>{t('windowDurationHours')}</label>
                    ...
                    <div style={{ marginBottom: 'var(--space-md)', background: 'white', padding: '10px', borderRadius: '4px', border: '1px solid #eee' }}>
                        <label style={{ fontSize: '0.9rem', fontWeight: 'bold', display: 'block', marginBottom: '8px' }}>{t('addFastingSlot')}</label>
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                            <div>
                                <label style={{ fontSize: '0.8rem', display: 'block' }}>{t('startDay')}</label>
                                <select value={tempStartDay} onChange={e => setTempStartDay(Number(e.target.value))} style={{ padding: '4px' }}>
                                    {[...Array(Math.ceil(windowDuration / 24))].map((_, i) => <option key={i} value={i}>{t('day')} {i + 1}</option>)}
                                </select>
                            </div>
                            <div>
                                <label style={{ fontSize: '0.8rem', display: 'block' }}>{t('startTime')}</label>
                                <input type="time" value={tempStartTime} onChange={e => setTempStartTime(e.target.value)} style={{ padding: '4px' }} />
                            </div>
                            <span style={{ paddingBottom: '6px' }}>{t('to')}</span>
                            <div>
                                <label style={{ fontSize: '0.8rem', display: 'block' }}>{t('endDay')}</label>
                                <select value={tempEndDay} onChange={e => setTempEndDay(Number(e.target.value))} style={{ padding: '4px' }}>
                                    {[...Array(Math.ceil(windowDuration / 24) + 1)].map((_, i) => <option key={i} value={i}>{t('day')} {i + 1}</option>)}
                                </select>
                            </div>
                            <div>
                                <label style={{ fontSize: '0.8rem', display: 'block' }}>{t('endTime')}</label>
                                <input type="time" value={tempEndTime} onChange={e => setTempEndTime(e.target.value)} style={{ padding: '4px' }} />
                            </div>
                            <button type="button" onClick={handleAddSlot} style={{ background: '#eee', border: 'none', padding: '6px 12px', cursor: 'pointer', borderRadius: '4px' }}>{t('add')}</button>
                        </div>
                    </div>

                    <div style={{ marginBottom: 'var(--space-md)' }}>
                        <h5>{t('definedSlots')}</h5>
                        {slots.length === 0 && <span style={{ color: '#999', fontSize: '0.9rem' }}>{t('noSlotsAdded')}</span>}
                        {slots.map((s, i) => (
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <button onClick={handleSave} style={{ flex: 1, padding: '8px', background: 'var(--c-primary)', color: 'white', border: 'none', borderRadius: 'var(--radius-md)', cursor: 'pointer' }}>
                            {editingTypeId ? t('saveChanges') : t('createType')}
                        </button>
                        <button onClick={resetForm} style={{ flex: 1, padding: '8px', background: 'transparent', border: '1px solid #ccc', borderRadius: 'var(--radius-md)', cursor: 'pointer' }}>{t('cancel')}</button>
                    </div>
                </div>
            )}
                ...
                {[...BUILT_IN_FASTING_TYPES, ...customFastingTypes].map((type) => (
                    <div key={type.id} style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            {!type.isSystem && (
                                <div>
                                    <button onClick={() => startEditing(type)} style={{ color: 'var(--c-primary)', background: 'transparent', border: 'none', cursor: 'pointer', marginRight: '8px', textDecoration: 'underline' }}>{t('edit')}</button>
                        ...
                        <div style={{ fontSize: '0.8rem', color: 'var(--c-text-muted)', marginTop: '4px' }}>
                            {t('window')}: {type.windowDuration || 24}h
                        </div>
